    ...
    $cart_products[] = '';
        ...
        if(mysqli_num_rows($select_cart) > 0){
            while($cart_item = mysqli_fetch_assoc($select_cart)){
                $query = "select * from products where id='".$cart_item['pid']."'";
                $result = mysqli_query($conn, $query);
                $row = mysqli_fetch_array($result);
                $cart_products[] = $row['productname'].' ('.$cart_item['quantity'].') ';
                $sub_total = ($row['productprice'] * $cart_item['quantity']);
...
<?php
        ...
        if(mysqli_num_rows($select_cart) > 0){
            while($fetch_cart = mysqli_fetch_assoc($select_cart)){
                $query = "select * from products where id='".$fetch_cart['pid']."'";
                $result = mysqli_query($conn, $query);
                $row = mysqli_fetch_array($result);
                $total_price = ($row['productprice'] * $fetch_cart['quantity']);
            $grand_total += $total_price;

    ?>
     
    <p> <?php echo $row['productname'] ?> <span>(<?php echo ' &#x20B9; '.$row['productprice'].' x '.$fetch_cart['quantity']  ?>)</span> </p>
    <?php
        }
        }else{
